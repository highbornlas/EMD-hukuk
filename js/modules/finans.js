...
function openSureModal(editId) {
  ...
  state.davalar.forEach(d => davaSel.innerHTML += `<option value="dava_${d.id}">📁 ${d.no||d.id.slice(0,6)} — ${escHTML(d.konu||'')} (${escHTML(getMuvAd(d.muvId))})</option>`);
  state.icra.forEach(i => davaSel.innerHTML += `<option value="icra_${i.id}">⚡ ${i.no||i.id.slice(0,6)} — ${escHTML(i.borclu||'')}</option>`);
